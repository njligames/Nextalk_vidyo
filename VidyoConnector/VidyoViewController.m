...
static NSString * _Nonnull const DEFAULT_LOG_LEVELS_AND_CATEGORIES = @"warning info@VidyoClient info@LmiPortalSession info@LmiPortalMembership info@LmiResourceManagerUpdates info@LmiPace info@LmiIce info@VidyoCameraEffect";
static NSString * _Nonnull const DEBUG_LOG_LEVELS_AND_CATEGORIES = @"warning debug@VidyoClient all@LmiPortalSession all@LmiPortalMembership info@LmiResourceManagerUpdates info@LmiPace info@LmiIce all@LmiSignaling info@VidyoCameraEffect";
...
@private
	...
	NSInteger     reconnectAttemptTimeout;
	BOOL          videoWasDisabled;
	NSTimer       *reconnectingTimer;
	BOOL          toolBarVisibility;
	VCConnectorCameraEffectType effectType;
	AVRoutePickerView* routePicker;
	BOOL          cameraWasInPrivacy;
}
@end

@implementation VidyoViewController

@synthesize toggleConnectButton, cameraPrivacyButton, microphonePrivacyButton, speakerButton;
...
- (void)viewDidLoad {
	...
	toolBarVisibility = NO;
	cameraWasInPrivacy = NO;
	effectType = VCConnectorCameraEffectTypeNone;
	...
	/*
	* By default, Google analytics service is started when connector object is constructed.
	* Need to stop Google analytics service for being started from sample application.
	* Application can use VidyoConnectorAnalyticsStart & VidyoConnectorAnalyticsStop api's in order to start & stop analytics service.
	*/
	NSString *constructOptions = @"{";
	constructOptions = [constructOptions stringByAppendingString:@"\"enableGoogleAnalytics\":false"];
	constructOptions = [constructOptions stringByAppendingString:@"}"];
	
	if(![VCConnectorPkg setConstructOptions:constructOptions.UTF8String]) {
		[logger Log:@"Failed to disable the google analytic service before connector is constructed"];
	}
	else {
		[logger Log:@"Disable the google analytic service before connector is constructed"];
	}
	
    const char * logLevels = [appSettings enableDebug] ? [DEBUG_LOG_LEVELS_AND_CATEGORIES UTF8String] : [DEFAULT_LOG_LEVELS_AND_CATEGORIES UTF8String];
    ...
    if (vc) {
        ...
        if ( ![vc registerLogEventListener:self Filter:logLevels] ) {
            [logger Log:@"LogEventListener registration failed."];
        }
		// Register for reconnect events
		if ( ![vc registerReconnectEventListener:self] ) {
			[logger Log:@"ReconnectEventListener registration failed."];
		}
		// Register for disable video on low BW events
		if ( ![vc registerResourceManagerEventListener:self] ) {
			[logger Log:@"ResourceManagerEventListener registration failed."];
		}
		// Register moderation command events
		if ( ![vc registerModerationCommandEventListener:self] ) {
			[logger Log:@"ModerationCommandEventListener registration failed."];
		}
	...
	routePicker = [[AVRoutePickerView alloc] initWithFrame:CGRectMake(0,0,40,40)];
	[routePicker setAlpha:0.1];
	[self->speakerButton addSubview:self->routePicker];
...
- (void)appWillResignActive:(NSNotification*)notification {
    if (vc) {
		cameraWasInPrivacy = [appSettings cameraPrivacy];
...
- (void)appDidBecomeActive:(NSNotification*)notification {
    if (vc) {
        ...
        [vc setCameraPrivacy:cameraWasInPrivacy];
    }
}
...
- (void)applyAppSettings {
	...
	[advancedOptions appendFormat:@"\"enableAutoReconnect\":%@,", [appSettings enableAutoReconnect] ? @"true" : @"false"];
	[advancedOptions appendFormat:@"\"disableVideoOnLowBw\":%@,", [appSettings disableVideoOnLowBw] ? @"true" : @"false"];
    [advancedOptions appendString:@"}"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        switch (self->vidyoConnectorState) {
            ...
            case VidyoConnectorStateFailureInvalidResource:
                ...
                [self->cameraPrivacyButton setEnabled:YES];
                [self->microphonePrivacyButton setEnabled:YES];
                ...
                if ([self->appSettings returnURL]) {
					...
					[[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@?callstate=%d", [self->appSettings returnURL], (int)(self->vidyoConnectorState == VidyoConnectorStateDisconnected)]] options:@{} completionHandler:nil];
                }
...
- (IBAction)microphonePrivacyButtonPressed:(id)sender {
	...
	if([vc setMicrophonePrivacy:microphonePrivacy]) {
		[microphonePrivacyButton setImage:[UIImage imageNamed:(microphonePrivacy ? @"microphoneOff.png" : @"microphoneOnWhite.png")] forState:UIControlStateNormal];
	}
	else {
		[appSettings toggleMicrophonePrivacy];
	}
}
...
- (IBAction)cameraPrivacyButtonPressed:(id)sender {
	...
	if([vc setCameraPrivacy:cameraPrivacy]) {
		[cameraPrivacyButton setImage:[UIImage imageNamed:( cameraPrivacy ? @"cameraOff.png" : @"cameraOnWhite.png")] forState:UIControlStateNormal];
	}
	else {
		[appSettings toggleCameraPrivacy];
	}
}
...
- (IBAction)toggleDebugButtonPressed:(id)sender {
    if ([appSettings toggleDebug]) {
		[vc enableDebug:7776 LogFilter:[DEBUG_LOG_LEVELS_AND_CATEGORIES UTF8String]];
        [clientVersion setHidden:NO];
    } else {
        [vc disableDebug];
...
- (IBAction)enableAutoReconnectPressed:(id)sender {
	appSettings.enableAutoReconnect = [_autoReconnect isOn];
    NSMutableString *advancedOptions = [NSMutableString stringWithString:@"{"];
	[advancedOptions appendFormat:@"\"enableAutoReconnect\":%@,", appSettings.enableAutoReconnect? @"true" : @"false"];
    [advancedOptions appendString:@"}"];
    [vc setAdvancedOptions:[advancedOptions UTF8String]];
...
- (IBAction)speakerButtonPressed:(id)sender {
	if([self->appSettings speakerPrivacy]){
		BOOL speakerPrivacy = [appSettings toggleSpeakerPrivacy];
		[self->speakerButton setImage:[UIImage imageNamed:(speakerPrivacy ? @"speakerOff.png" : @"speakerOn.png")] forState:UIControlStateNormal];
		[vc setSpeakerPrivacy:speakerPrivacy];
	}
}

- (IBAction)toggleCameraEffectButtonPressed:(id)sender {
	VCConnectorCameraEffectInfo* info = [[VCConnectorCameraEffectInfo alloc] init];
	
	switch (effectType) {
		case VCConnectorCameraEffectTypeNone:
			effectType = VCConnectorCameraEffectTypeBlur;
			info.token = [NSMutableString stringWithFormat:@"%s", BNB_TOKEN_STR];
			info.pathToResources = [NSMutableString stringWithFormat:@"%@/Frameworks/BNBEffectPlayerC.framework/bnb-resources", [[NSBundle mainBundle] resourcePath]];
			info.pathToEffect = [NSMutableString stringWithFormat:@"%@/effects/blurred-background", [[NSBundle mainBundle] resourcePath]];
			info.virtualBackgroundPicture = [[NSMutableString alloc] init];
			info.blurIntensity = 5;
			break;
		case VCConnectorCameraEffectTypeBlur:
			effectType = VCConnectorCameraEffectTypeVirtualBackground;
			info.token = [NSMutableString stringWithFormat:@"%s", BNB_TOKEN_STR];
			info.pathToResources = [NSMutableString stringWithFormat:@"%@/Frameworks/BNBEffectPlayerC.framework/bnb-resources", [[NSBundle mainBundle] resourcePath]];
			info.pathToEffect = [NSMutableString stringWithFormat:@"%@/effects/virtual-background", [[NSBundle mainBundle] resourcePath]];
			info.virtualBackgroundPicture = [[NSMutableString alloc] init];
			break;
		case VCConnectorCameraEffectTypeVirtualBackground:
			effectType = VCConnectorCameraEffectTypeNone;
			info.token = [[NSMutableString alloc] init];
			info.pathToResources = [[NSMutableString alloc] init];
			info.pathToEffect = [[NSMutableString alloc] init];
			info.virtualBackgroundPicture = [[NSMutableString alloc] init];
			break;
		default:
			break;
	...
	[vc setCameraBackgroundEffect:info];
}

- (IBAction)disableVideoOnLowBWPressed:(id)sender {
	appSettings.disableVideoOnLowBw = [_disableVideo  isOn];
    NSMutableString *advancedOptions = [NSMutableString stringWithString:@"{"];
	[advancedOptions appendFormat:@"\"disableVideoOnLowBw\":%@,", appSettings.disableVideoOnLowBw ? @"true" : @"false"];
    [advancedOptions appendString:@"}"];
    [vc setAdvancedOptions:[advancedOptions UTF8String]];
...
-(void) onLocalCameraStateUpdated:(VCLocalCamera*)localCamera State:(VCDeviceState)state {
	...
	dispatch_async( dispatch_get_main_queue(), ^{
		BOOL cameraPrivacy = NO;
		switch (state) {
			case VCDeviceStateStopped:
			case VCDeviceStatePaused:
				cameraPrivacy = YES;
				break;
			case VCDeviceStateStarted:
			case VCDeviceStateResumed:
				cameraPrivacy = NO;
				break;
			default:
				return;
		}
		[self->appSettings setCameraPrivacy:cameraPrivacy];
		[self->cameraPrivacyButton setImage:[UIImage imageNamed:(cameraPrivacy ? @"cameraOff.png" : @"cameraOnWhite.png")] forState:UIControlStateNormal];
	} );
}
...
-(void) onLocalMicrophoneStateUpdated:(VCLocalMicrophone*)localMicrophone State:(VCDeviceState)state {
	...
	dispatch_async( dispatch_get_main_queue(), ^{
		BOOL microphonePrivacy = NO;
		switch (state) {
			case VCDeviceStateStopped:
			case VCDeviceStatePaused:
				microphonePrivacy = YES;
				break;
			case VCDeviceStateStarted:
			case VCDeviceStateResumed:
		...
		[self->appSettings setMicrophonePrivacy:microphonePrivacy];
		[self->microphonePrivacyButton setImage:[UIImage imageNamed:(microphonePrivacy ? @"microphoneOff.png" : @"microphoneOnWhite.png")] forState:UIControlStateNormal];
	} );
}
...
-(void) onLocalSpeakerStateUpdated:(VCLocalSpeaker*)localSpeaker State:(VCDeviceState)state {
	...
	dispatch_async( dispatch_get_main_queue(), ^{
		BOOL speakerPrivacy = NO;
		if(state == VCDeviceStateStopped) {
			speakerPrivacy = YES;
		}
		else if(state == VCDeviceStateStarted) {
			speakerPrivacy = NO;
		} else {
			return;
		}
		...
		[self->appSettings setSpeakerPrivacy:speakerPrivacy];
		[self->speakerButton setImage:[UIImage imageNamed:(speakerPrivacy ? @"speakerOff.png" : @"speakerOn.png")] forState:UIControlStateNormal];
	} );
}
...
{
	[logger Log:[NSString stringWithFormat:@"onModerationCommandReceived: deviceType=%ld moderationType=%ld state=%d", deviceType, moderationType, state]];
	dispatch_async( dispatch_get_main_queue(), ^{
		if(moderationType == VCRoomModerationTypeHardMute) {
			if(deviceType == VCDeviceTypeLocalCamera) {
				[self->cameraPrivacyButton setEnabled:!state];
			}
			else if(deviceType == VCDeviceTypeLocalMicrophone) {
				[self->microphonePrivacyButton setEnabled:!state];
			}
		}
	} );
}
...
#pragma mark - VCConnectorIRegisterReconnectEventListener

-(void) onReconnectingStatusUpdate{
	dispatch_async( dispatch_get_main_queue(), ^{
		if(self->reconnectAttemptTimeout <= 0) {
			[self->toolbarStatusText setText:@"Reconnecting..."];
			[self->reconnectingTimer invalidate];
			self->reconnectingTimer = nil;
		}
		else {
			[self->toolbarStatusText setText:[[NSString alloc] initWithFormat:@"Will reconnect in %ld", (long)self->reconnectAttemptTimeout]];
			self->reconnectAttemptTimeout--;
		}
    } );
}
...
{
	dispatch_async( dispatch_get_main_queue(), ^{
		self->toolBarVisibility = self->toolbarView.hidden;
		self->toolbarView.hidden = NO;
		self->reconnectAttemptTimeout = attemptTimeout;
		[self->toolbarStatusText setText:[[NSString alloc] initWithFormat:@"Will reconnect in %ld", (long)self->reconnectAttemptTimeout]];
		self->reconnectAttemptTimeout--;
		if(self->reconnectingTimer) {
			[self->reconnectingTimer invalidate];
			self->reconnectingTimer = nil;
		}
		self->reconnectingTimer = [NSTimer scheduledTimerWithTimeInterval: 1.0
																target: self
																selector:@selector(onReconnectingStatusUpdate)
...
{
	dispatch_async( dispatch_get_main_queue(), ^{
		[self->toolbarStatusText setText:@"Reconnected"];
    } );
	if(self->toolBarVisibility) {
...
{
	dispatch_async( dispatch_get_main_queue(), ^{
		[self->toolbarStatusText setText:@"Conference lost"];
    } );
}
...
{
	if((videoWasDisabled && !maxRemoteSources) ||
	   (!videoWasDisabled && maxRemoteSources)) {
		return;
	}
	dispatch_async( dispatch_get_main_queue(), ^{
		
		NSString* statusText = maxRemoteSources == 0 ? @"The video was disabled due to poor connection" : @"The video has been restored due to connection improvements";
		self->videoWasDisabled = maxRemoteSources == 0;
		
		[self->_disableVideoLable setText:statusText];
		[self->_disableVideoLable setHidden:NO];
		
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 3.0 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
			[self->_disableVideoLable setHidden:YES];
		 });
    } );
}
